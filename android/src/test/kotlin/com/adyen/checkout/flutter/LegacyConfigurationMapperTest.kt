...
class LegacyConfigurationMapperTest {
    private val context: Context = mock()
    private val TEST_CLIENT_KEY = "test_qwertyuiopasdfghjklzxcvbnmqwerty"

    private fun CardComponentConfigurationDTO.mapCardConfiguration(): CardConfiguration {
        val checkoutConfiguration = this.toCheckoutConfiguration()
        return checkoutConfiguration.getConfiguration(com.adyen.checkout.components.core.PaymentMethodTypes.SCHEME)!!
    }

    private fun AmountDTO.toAmount(): Amount = Amount(currency, value)
    ...
    inner class DropInConfigurationTests {
        ...
        fun `when minimal dropin configuration is provided, then map correctly`() {
            ...
            assertEquals(SDKEnvironment.TEST, dropInConfiguration.environment)
            assertEquals(TEST_CLIENT_KEY, dropInConfiguration.clientKey)
        }
        ...
        fun `when dropin configuration DTO is provided, then map it to native dropin configuration model`() {
            ...
            assertEquals(SDKEnvironment.TEST, dropInConfiguration.environment)
            assertEquals(TEST_CLIENT_KEY, dropInConfiguration.clientKey)
            assertEquals("en-US", dropInConfiguration.shopperLocale?.toLanguageTag())
            assertEquals("USD", dropInConfiguration.amount?.currency)
            assertEquals(1824, dropInConfiguration.amount?.value)
//            assertEquals(false, dropInConfiguration.preselectedStoredPaymentMethod)
...
//            assertEquals(true, dropInConfiguration.isRemovingStoredPaymentMethodsEnabled)
        ...
        fun `when dropin configuration has null optional fields, then map correctly`() {
            ...
            assertEquals(SDKEnvironment.TEST, dropInConfiguration.environment)
            assertNull(dropInConfiguration.shopperLocale)
            assertNull(dropInConfiguration.amount)
        }
    }
    ...
    inner class CardConfigurationTests {
        ...
        fun `when card configuration has address mode FULL, then map to FullAddress`() {
            ...
            val cardConfiguration = cardComponentConfigurationDTO.mapCardConfiguration()
        ...
        @Test
        fun `when card configuration has unknown and null card types, then filter them out`() {
            val cardConfigurationDTO = CardConfigurationDTO(
                holderNameRequired = false,
                addressMode = AddressMode.NONE,
                showStorePaymentField = true,
                ...
                kcpFieldVisibility = FieldVisibility.HIDE,
                socialSecurityNumberFieldVisibility = FieldVisibility.HIDE,
                supportedCardTypes = listOf("visa", null, "not_a_real_brand")
            )
            val cardComponentConfigurationDTO = CardComponentConfigurationDTO(
                environment = Environment.TEST,
                clientKey = TEST_CLIENT_KEY,
            ...
            assertEquals(1, cardConfiguration.supportedCardBrands?.size)
        }
    ...
    inner class ThreeDS2ConfigurationTests {
        ...
        fun `when 3DS2 configuration is null, then no 3DS2 config is added`() {
            ...
            assertNotNull(cardConfiguration)
        }

        @Test
        fun `when 3DS2 configuration is provided, then map requestorAppURL`() {
            val requestorAppURL = "myapp://3ds2/requestor"
            val cardComponentConfigurationDTO = CardComponentConfigurationDTO(
                environment = Environment.TEST,
                clientKey = TEST_CLIENT_KEY,
                countryCode = "US",
                ...
                threeDS2ConfigurationDTO = com.adyen.checkout.flutter.generated.ThreeDS2ConfigurationDTO(
    ...
    inner class CashAppPayConfigurationTests {
        ...
        fun `when Cash App Pay configuration with SANDBOX environment is provided, then map correctly`() {
            ...
            assertNotNull(dropInConfiguration)
        }

        @Test
        fun `when Cash App Pay configuration with PRODUCTION environment is provided, then map correctly`() {
            val returnUrl = "myapp://cashapp/callback"
            val dropInConfigurationDTO = DropInConfigurationDTO(
                environment = Environment.TEST,
                clientKey = TEST_CLIENT_KEY,
                countryCode = "US",
    ...
    inner class AnalyticsConfigurationTests {
        ...
        fun `when analytics is enabled, then analytics level is ALL`() {
            ...
            assertEquals(
                ...
                dropInConfiguration.analyticsConfiguration?.level
            )
        }
