...
// TODO: v6 migration - DropInComponent and related V1 types are now package-access.
// Drop-in needs full migration to Checkout.setup() + createDropIn() when v6 exposes it publicly.
@MainActor
...
//    }
    ...
    private func sendDropInUnavailableError() {
        let checkoutEvent = CheckoutEvent(
            type: CheckoutEventType.result,
            data: PaymentResultDTO(
                type: PaymentResultEnum.error,
                reason: "Drop-in is not yet available in iOS SDK v6. Use component-based integration instead."
            )
        )
        checkoutFlutter.send(event: checkoutEvent, completion: { _ in })
    }
