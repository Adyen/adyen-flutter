...
- For Drop-in on iOS: simplified stored payment method deletion handling by consolidating duplicated
  internal delegates.
- For Drop-in on iOS: internal delegate implementations are now organized into focused files, and
  balance-check event serialization failures no longer retain the completion handler.
